...
async function createScene() {
  ...
  if (!navigator.xr) {
    statusText.textContent = 'Model ready · start the browser camera';
    return scene;
  }
...
async function startCamera() {
  ...
  try {
    ...
    statusText.textContent = 'Live browser camera · drag or pinch the model';
  } catch (error) {
    cameraButton.disabled = false;
    statusText.textContent = error.name === 'NotAllowedError'
      ? 'Camera blocked — allow it in browser settings and retry'
